...
class PIDController(AbstractController):
    ...
    Kp: float = Field(..., gt=0, description="Proportional gain")
    Ti: Seconds = Field(default=float("inf"), gt=0, description="Integral time constant")
    Td: Seconds = Field(default=0.0, ge=0, description="Derivative time constant")
    ...
    velocity_form: bool = Field(
        default=False, description=("Whether to use the velocity form of the PID formulation.")
    )
    # additional PID only private attributes
    _error_queue: deque[StateValue] = PrivateAttr(
    ...
    def _control_algorithm(
        ...
        if self.velocity_form:
            ...
            if overflow > 0 and self.Ti != float("inf"):
                ...
                self._integral += (
                    -overflow * self.Ti / self.Kp
                )  # since overflow > 0, this decreases integral.
                saturated = "Overflow"
            if underflow < 0 and self.Ti != float("inf"):
                # we are underflowing the range, increase integral and output to match lower range
                output -= underflow
                self._integral += (
                    -underflow * self.Ti / self.Kp
                )  # since underflow < 0, this increases integral.
                saturated = "Underflow"

...
from modular_simulation.utils.wrappers import second, per_second
...
class Trajectory:
    ...
    def __call__(self, t: Seconds | TimeQuantity) -> StateValue:
        """
    ...
    def set(self, value: StateValue, t: Seconds | TimeQuantity | None = None) -> Trajectory:
        """
        ...
        """
        ts = second(t) if t is not None else self.end_time_s()
        self._clip_at(ts)
    ...
    @overload
    def ramp(
        self, magnitude: float, *, duration: Seconds | TimeQuantity, rate: None = None
    ) -> Trajectory: ...
    @overload
    def ramp(
        self, magnitude: float, *, duration: None = None, rate: PerSeconds | PerTimeQuantity
    ) -> Trajectory: ...
    ...
    def ramp(
        ...
        *,
        ...
        if duration is not None:
            ...
            dur_s = abs(float(magnitude) / per_second(rate))
        else:
    ...
    def hold(self, duration: Seconds | TimeQuantity, value: StateValue | None = None) -> Trajectory:
        """
    ...
    def random_walk(
        ...
        *,
        ...
        dt: Seconds | TimeQuantity = 1.0,
        clamp: tuple[StateValue | None, StateValue | None] = (None, None),
        seed: int = 0,
    ) -> Trajectory:
        """
    ...
    def start(self) -> tuple[Seconds, StateValue]:
        return self._knots_t[0], self._knots_y[0]

    def end(self) -> tuple[Seconds, StateValue]:
